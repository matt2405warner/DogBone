...
static void ImGui_ImplGlfw_UpdateMousePosAndButtons()
{
    ...
    if (focused)
    {
        ...
        {
            glfwSetCursorPos(g_Window, static_cast<double>(mouse_pos_backup.x),
...
void ImGui_ImplGlfw_NewFrame()
{
    ...
    float f_w = static_cast<float>(w);
    float f_h = static_cast<float>(h);
    float f_display_w = static_cast<float>(display_w);
    float f_display_h = static_cast<float>(display_h);

    io.DisplaySize = ImVec2(f_w,f_h);
    io.DisplayFramebufferScale = ImVec2(f_w > 0 ? (f_display_w / f_w) : 0, f_h > 0 ? (f_display_h / f_h) : 0);
