...
// / programmatic pipeline) This needs to be used along with a Platform Binding (e.g.
// GLFW, SDL, Win32, custom..) (Note: We are using GL3W as a helper library to access
// OpenGL functions since there is no standard header to access modern OpenGL functions
// easily. Alternatives are GLEW, Glad, etc..)
...
//  2018-11-30: Misc: Setting up io.BackendRendererName so it can be displayed in the
//  About Window. 2018-11-13: OpenGL: Support for GL 4.5's glClipControl(GL_UPPER_LEFT).
//  2018-08-29: OpenGL: Added support for more OpenGL loaders: glew and glad, with
//  comments indicative that any loader can be used. 2018-08-09: OpenGL: Default to OpenGL
//  ES 3 on iOS and Android. GLSL version default to "#version 300 ES". 2018-07-30:
//  OpenGL: Support for GLSL 300 ES and 410 core. Fixes for Emscripten compilation.
//  2018-07-10: OpenGL: Support for more GLSL versions (based on the GLSL version string).
//  Added error output when shaders fail to compile/link. 2018-06-08: Misc: Extracted
//  imgui_impl_opengl3.cpp/.h away from the old combined GLFW/SDL+OpenGL3 examples.
//  2018-06-08: OpenGL: Use draw_data->DisplayPos and draw_data->DisplaySize to setup
//  projection matrix and clipping rectangle. 2018-05-25: OpenGL: Removed unnecessary
//  backup/restore of GL_ELEMENT_ARRAY_BUFFER_BINDING since this is part of the VAO state.
//  2018-05-14: OpenGL: Making the call to glBindSampler() optional so 3.2 context won't
//  fail if the function is a NULL pointer. 2018-03-06: OpenGL: Added const char*
//  glsl_version parameter to ImGui_ImplOpenGL3_Init() so user can override the GLSL
//  version e.g. "#version 150". 2018-02-23: OpenGL: Create the VAO in the render function
//  so the setup can more easily be used with multiple shared GL context. 2018-02-16:
...
#endif
    IM_ASSERT(
            static_cast<int>(strlen(glsl_version)) + 2 <
...
ImGui_ImplOpenGL3_RenderDrawData(ImDrawData *draw_data)
{
    ...
    ImGuiIO &io = ImGui::GetIO();
    int fb_width =
            static_cast<int>(draw_data->DisplaySize.x * io.DisplayFramebufferScale.x);
    int fb_height =
    ...
    glGetIntegerv(
            GL_BLEND_EQUATION_RGB, reinterpret_cast<GLint *>(&last_blend_equation_rgb));
    ...
    glGetIntegerv(
            GL_CLIP_ORIGIN,
            reinterpret_cast<GLint *>(&last_clip_origin)); // Support for GL 4.5's
    ...
    // contexts, and we don't track creation/deletion of windows so we don't have an
    // obvious key to use to cache them.)
    GLuint vao_handle = 0;
    glGenVertexArrays(1, &vao_handle);
    glBindVertexArray(vao_handle);
    glBindBuffer(GL_ARRAY_BUFFER, g_VboHandle);
    glEnableVertexAttribArray(static_cast<GLuint>(g_AttribLocationPosition));
    glEnableVertexAttribArray(static_cast<GLuint>(g_AttribLocationUV));
    glEnableVertexAttribArray(static_cast<GLuint>(g_AttribLocationColor));
    glVertexAttribPointer(
            g_AttribLocationPosition, 2, GL_FLOAT, GL_FALSE, sizeof(ImDrawVert),
            reinterpret_cast<GLvoid *>(IM_OFFSETOF(ImDrawVert, pos)));
    glVertexAttribPointer(
            g_AttribLocationUV, 2, GL_FLOAT, GL_FALSE, sizeof(ImDrawVert),
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ImVec4 clip_rect = ImVec4(
                        pcmd->ClipRect.x - pos.x, pcmd->ClipRect.y - pos.y,
                    ...
                    clip_rect.w >= 0.0f)
                {
                    ...
                    if (clip_origin_lower_left)
                        glScissor(
                                ...
                                static_cast<int>(
                                        (static_cast<float>(fb_height) - clip_rect.w)),
                        ...
                        glScissor(
                                ...
                                static_cast<int>(
                                        clip_rect.w)); // Support for GL 4.5's
                                                       // glClipControl(GL_UPPER_LEFT)

                    // Bind texture, Draw
                    glBindTexture(
                            GL_TEXTURE_2D, static_cast<GLuint>(reinterpret_cast<intptr_t>(
                                                   pcmd->TextureId)));
                    glDrawElements(
...
#ifdef GL_SAMPLER_BINDING
    glBindSampler(0, static_cast<GLuint>(last_sampler));
#endif
    ...
    glBlendFuncSeparate(
            last_blend_src_rgb, last_blend_dst_rgb, last_blend_src_alpha,
    ...
    glScissor(
            last_scissor_box[0], last_scissor_box[1], last_scissor_box[2],
            last_scissor_box[3]);
}
...
CheckShader(GLuint handle, const char *desc)
{
    ...
    if (log_length > 0)
    {
        ...
        glGetShaderInfoLog(
                handle, log_length, nullptr, buf.begin());
        fprintf(stderr, "%s\n", buf.begin());
    }
...
ImGui_ImplOpenGL3_CreateDeviceObjects()
{
    ...
    const GLchar *fragment_shader_with_version[2] = {
            g_GlslVersionString, fragment_shader};
